...
{
    public class frctrimestermeadim2Controller : ApiController
    {
        public HttpResponseMessage Post(frc a)
        {

            string query =
                @"select non  empty{(" + a.selectmeasurerows + "," + a.selectdimentionrows + ".members)} on rows," +
                "filter([Dim Calendar].[trimester].members," + a.selectmeasurefilter + "" + a.condition + ")on columns from [BI]";
