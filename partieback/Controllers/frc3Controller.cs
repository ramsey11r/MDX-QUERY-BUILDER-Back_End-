...
{
    public class frc3Controller : ApiController
    {

        public HttpResponseMessage Post(frc a)
        {
            string query = @"select  non empty{
                (" + a.selectdimentionrows + ".members," + a.selectmeasurerows + ")} on rows,non empty (" + a.selectdimentioncolumns + ".members) on columns from [BI]";
