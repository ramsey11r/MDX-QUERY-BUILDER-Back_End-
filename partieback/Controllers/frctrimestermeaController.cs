...
{
    public class frctrimestermeaController : ApiController
    {
        ...
        public HttpResponseMessage Post(frc a)
        {
            ...
            string query =
                @"select non  empty{" + a.selectmeasurecolumns + "} on columns," +
                "filter([Dim Calendar].[Trimester].members," + a.selectmeasurefilter + "" + a.condition + ")on rows from [BI]";
