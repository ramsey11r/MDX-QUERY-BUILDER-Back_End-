...
{
    public class frcweekController : ApiController
    {

        // angular -> backend -> controller -> loginc ->base de donné -> Model
        public HttpResponseMessage Post(frc a)
        {

            string query =
                @"select non  empty(" + a.selectdimentioncolumns + ".members) on columns," +
                "filter([Dim Calendar].[week].members," + a.selectmeasurefilter + "" + a.condition + ")on rows from [BI]";
