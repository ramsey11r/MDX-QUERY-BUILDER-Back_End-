...
namespace partieback.Controllers {
    public class frcController : ApiController
    {

        public HttpResponseMessage Post(frc a )
        {
            string query = @"select non  empty{" + a.selectmeasurerows + "} on rows, non empty(" + a.selectdimentioncolumns + ".members) on columns from [BI]";
